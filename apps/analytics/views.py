...
#     # Get exam performances
#     exam_performances = ExamPerformance.objects.filter(user=request.user).select_related('exam')[:5]
...
#     # Get recent activity
#     recent_activities = DailyActivity.objects.filter(user=request.user)[:7]

#     # Get recent test attempts
#     recent_attempts = TestAttempt.objects.filter(
#         user=request.user,
#         status='completed'
#     ).select_related('mock_test')[:10]
...
#     # Get subject performances for this exam
#     subject_performances = SubjectPerformance.objects.filter(
#         user=request.user,
#         subject__exam=exam
#     ).select_related('subject')

#     # Get topic performances for this exam
#     topic_performances = TopicPerformance.objects.filter(
#         user=request.user,
#         topic__subject__exam=exam
#     ).select_related('topic__subject')

#     # Get attempt history
#     attempts = TestAttempt.objects.filter(
#         user=request.user,
#         mock_test__exam=exam,
#         status='completed'
#     ).order_by('-end_time')[:20]
...
# @login_required
# def get_performance_data(request):
#     """API endpoint for performance data (charts)"""
#     # Get all attempts
#     attempts = TestAttempt.objects.filter(
#         user=request.user,
#         status='completed'
...
#     for attempt in attempts:
#         chart_data['labels'].append(attempt.mock_test.name[:20])
#         chart_data['scores'].append(attempt.percentage)
#         chart_data['accuracy'].append(attempt.percentage)
#         chart_data['dates'].append(attempt.end_time.strftime('%b %d'))

#     # Get subject-wise performance
#     subject_perf = SubjectPerformance.objects.filter(
#         user=request.user
#     ).select_related('subject')

#     subject_data = {
#         'labels': [sp.subject.name for sp in subject_perf],
#         'accuracy': [sp.accuracy for sp in subject_perf],
#         'questions': [sp.total_questions for sp in subject_perf]
#     }

#     # Get topic-wise weak areas
#     weak_topics = TopicPerformance.objects.filter(
#         user=request.user,
#         is_weak=True
#     ).select_related('topic')[:10]
...
#     weak_data = [
#         {'name': wt.topic.name, 'accuracy': wt.accuracy, 'questions': wt.total_questions}
#         for wt in weak_topics
#     ]

#     # Get exam-wise performance
#     exam_perf = ExamPerformance.objects.filter(
#         user=request.user
#     ).select_related('exam')

#     exam_data = {
#         'labels': [ep.exam.name for ep in exam_perf],
#         'accuracy': [ep.accuracy for ep in exam_perf],
#         'best_score': [ep.best_score for ep in exam_perf]
#     }
...
#         'exam_data': exam_data,
#         'total_tests': attempts.count(),
#         'overall_accuracy': request.user.performance.overall_accuracy if hasattr(request.user, 'performance') else 0,
#     })
...
#     """Get detailed analysis for a specific attempt"""
#     attempt = get_object_or_404(TestAttempt, id=attempt_id, user=request.user)
#     answers = TestAnswer.objects.filter(attempt=attempt).select_related('question', 'question__subject', 'question__topic')
...
#         subject = answer.question.subject.name
#         if subject not in subject_breakdown:
#             subject_breakdown[subject] = {'correct': 0, 'wrong': 0, 'skipped': 0, 'total': 0}
...
# @login_required
# def update_all_stats(request):
#     """Manual trigger to update all user stats"""
#     # Update user performance
#     performance, _ = UserPerformance.objects.get_or_create(user=request.user)
#     performance.update_stats()
...
#     exams = Exam.objects.filter(is_active=True)
#     for exam in exams:
#         exam_perf, _ = ExamPerformance.objects.get_or_create(user=request.user, exam=exam)
#         exam_perf.update_stats()

#     # Update subject performances
#     subjects = Subject.objects.filter(is_active=True)
#     for subject in subjects:
#         subject_perf, _ = SubjectPerformance.objects.get_or_create(user=request.user, subject=subject)
#         subject_perf.update_stats()

#     # Update topic performances
#     topics = Topic.objects.filter(is_active=True)
#     for topic in topics:
#         topic_perf, _ = TopicPerformance.objects.get_or_create(user=request.user, topic=topic)
#         topic_perf.update_stats()

#     return JsonResponse({'status': 'success', 'message': 'All stats updated successfully'})


# @login_required
# def get_recommendations(request):
#     """Get AI-powered recommendations based on weak areas"""
#     weak_topics = TopicPerformance.objects.filter(
#         user=request.user,
#         is_weak=True
...
#             'subject': 'General',
#             'accuracy': 100,
#             'suggestion': 'Great job! Keep practicing to maintain your performance.',
#             'practice_url': '/mock-tests/'
#         })
